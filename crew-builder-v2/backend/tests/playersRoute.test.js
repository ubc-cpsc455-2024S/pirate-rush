...
describe('PATCH /players/:playerId/berries', () => {
  ...
  it('should return 404 if the player is not found', async () => {
    ...
    const response = await request(app).patch(`/players/${playerId}/berries`).send({ amount: 20 })
  ...
  it('should return 500 if there is a server error', async () => {
    ...
    expect(response.body).toEqual({ message: 'Server error' })
    expect(db.collection().findOne).toHaveBeenCalledWith({ playerId })
    expect(db.collection().updateOne).toHaveBeenCalledWith({ playerId }, { $set: { berries: 120 } })
  })
})
